...
class Game
{
    ...
    constructor(player, sceneWorld, scene, world, chunkManager, network_manager=new NetworkManager("ws://localhost:5173")) { // Don't think we should be passing player here, think it should be generated by a method of the class
        this.player = player;
        ...
        this.tick_counter = 0;
        this.player_body = new CANNON.Body({ mass: 0, material: this.sceneWorld.getVoxelMat() });
        this.setup()
    }
    ...
    renderChunks(chunk) {
        ...
        if(instancedMesh.count !== 0){
            this.chunkManager.storeChunk(location, instancedMesh);
        }
...
const chunkManager = new ChunkRendererManager(scene, world);
let game = new Game(player, sceneWorld, scene, world, chunkManager);
game.start();
animate();
websocketConnect();
